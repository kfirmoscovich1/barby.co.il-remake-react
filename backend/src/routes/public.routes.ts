...
/**
 * PERFORMANCE: Middleware to add cache headers for public routes
 * - Cache-Control: Enables browser and CDN caching
 * - stale-while-revalidate: Serves stale content while fetching fresh
 * 
 * @param maxAge - Cache duration in seconds
 */
function setCacheHeaders(maxAge: number) {
    return (_req: Request, res: Response, next: NextFunction) => {
        // PERFORMANCE: Enable caching with stale-while-revalidate pattern
        // Browser/CDN can serve stale content while fetching fresh in background
        res.setHeader('Cache-Control', `public, max-age=${maxAge}, stale-while-revalidate=${maxAge * 2}`);
        // PERFORMANCE: Vary by Accept-Encoding for proper CDN caching of gzipped content
...
router.get(
    '/shows',
    setCacheHeaders(60), // PERFORMANCE: 1 minute cache (increased from 30s)
...
router.get('/shows/featured', setCacheHeaders(120), async (req, res, next) => {
    try {
        // PERFORMANCE: Default limit prevents unbounded queries
...
router.get('/shows/upcoming', setCacheHeaders(120), async (req, res, next) => {
    try {
        // PERFORMANCE: Default limit prevents unbounded queries, max 50
