/**
 * @file show.service.ts
 * @description Show service with performance optimizations.
 * 
 * PERFORMANCE OPTIMIZATIONS:
 * 1. In-memory cache for common queries (30s TTL)
 * 2. Field projection - only select needed fields
 * 3. Compound indexes used for all queries
 * 4. Auto-archive throttled to prevent frequent DB writes
 * 5. Lean queries for reduced memory usage
 */
...
// PERFORMANCE: Cache for auto-archive to prevent running on every request
let lastAutoArchiveRun = 0;
const AUTO_ARCHIVE_INTERVAL = 1000 * 60 * 60; // Run at most once per hour

/**
 * PERFORMANCE: Auto-archive shows with past dates (yesterday and before)
 * Throttled to run at most once per hour to reduce DB write load
 */
export async function autoArchivePastShows(): Promise<number> {
    ...
    if (result.modifiedCount > 0) {
        console.log(`📦 Auto-archived ${result.modifiedCount} past shows`);
    }

    return result.modifiedCount;
}

// PERFORMANCE: In-memory cache for shows list (short-lived)
...
const SHOWS_CACHE_TTL = 1000 * 60; // PERFORMANCE: 60 seconds cache (increased from 30s)

/**
 * Clear the shows cache (called after mutations)
 */
