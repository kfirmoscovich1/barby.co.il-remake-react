...
// Main shows list query (public - non-archived, published, sorted by date)
showSchema.index({ archived: 1, published: 1, dateISO: 1 });
// Featured shows query (featured, published, non-archived, sorted by date)
showSchema.index({ featured: 1, published: 1, archived: 1, dateISO: 1 });
// Lookup by slug (detail page)
showSchema.index({ slug: 1 });
// PERFORMANCE: Admin list query (may include unpublished, sorted by date)
showSchema.index({ archived: 1, dateISO: -1 });
// PERFORMANCE: Date range queries with status filter
showSchema.index({ dateISO: 1, status: 1 });
